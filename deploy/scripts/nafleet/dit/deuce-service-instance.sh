...
stack_action=$1
awsEnvironment="dev" # ["dev", "stage", "prod"]
...
appInfrastructureStackName="${platformName}-${instanceName}-${applicationName}-base"
...
stackName="${platformName}-${instanceName}-${applicationName}-instance"
...
aws cloudformation ${scriptTask}-stack --region ${awsRegion} --profile ${awsProfile} ${stackPolicy} \
    ...
    --parameters \
      ...
      ParameterKey=envType,ParameterValue=${awsEnvironment} \
      ParameterKey=envInstance,ParameterValue=${instanceName} \
