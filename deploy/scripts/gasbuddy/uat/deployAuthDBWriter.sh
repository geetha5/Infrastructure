...
if [[ "${type}" == "create" ]]; then 
  ...
  aws cloudformation create-stack --region us-east-1 --stack-name gasbuddy-authdbwriter-stage --template-url ${templateUrl} --capabilities CAPABILITY_IAM --capabilities CAPABILITY_NAMED_IAM --parameters \
  ...
  ParameterKey=dbHostname,ParameterValue= \
  ...
  aws cloudformation update-stack --region us-east-1 --stack-name gasbuddy-authdbwriter-stage --template-url ${templateUrl} --capabilities CAPABILITY_IAM --capabilities CAPABILITY_NAMED_IAM --parameters \
  ParameterKey=environment,ParameterValue=stage \
  ParameterKey=kinesisStackName,ParameterValue=gasbuddy-kinesis-stage \
  ParameterKey=kmsStackName,ParameterValue=gasbuddy-kms-stage \
  ParameterKey=vpcStackName,ParameterValue=mobile-stage-vpc \
  ...
  ParameterKey=batchSize,ParameterValue=50 \
  ParameterKey=rdsSecurityGroupId,ParameterValue=sg-45ef7030 \
  ParameterKey=dbHostname,ParameterValue=stage-auth-db.cietgyffq9mm.us-east-1.rds.amazonaws.com \
  ParameterKey=dbUser,ParameterValue=stage_admin \
  ParameterKey=dbPass,ParameterValue="a1h\$3Uh843x2" \
  --output text --profile mobile-stage
