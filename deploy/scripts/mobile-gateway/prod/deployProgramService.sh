...
templateUrl="https://s3.amazonaws.com/wex-mobile-devops/projects/mobile-gateway/releases/${version}/cloudformation/program-service.cf.json"
...
dnsName="program-service.wexmobileauth.com"
ansibleRoleVersion="releases/2.3.0"
projectVersion="releases/${version}"
appVersion="1.1.0-release"
deploymentId="238sdflhssdfsdfdflkjldfsdf9sdf"


if [[ "${type}" == "create" ]]; then

  aws cloudformation create-stack --region us-east-1 --stack-name program-service-prod --template-url ${templateUrl} \
  ...
  aws cloudformation update-stack --region us-east-1 --stack-name program-service-prod --template-url ${templateUrl} \
  --capabilities CAPABILITY_IAM --capabilities CAPABILITY_NAMED_IAM --parameters \
  ParameterKey=environment,ParameterValue=${environment} \
  ParameterKey=vpcStackName,ParameterValue=${vpcStackName} \
  ParameterKey=albCommonStackName,ParameterValue=${albCommonStackName} \
