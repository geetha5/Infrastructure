...
templateUrl="https://s3.amazonaws.com/wex-mobile-devops/projects/mobile-gateway/releases/${version}/cloudformation/clientidentity-service.cf.json"
...
dnsName="client-identity-service.wexmobileauth.com"
...
if [[ "${type}" == "create" ]]; then
  ...
  aws cloudformation create-stack --region us-east-1 --stack-name clientidentity-service-prod --template-url ${templateUrl} \
  ...
  aws cloudformation update-stack --region us-east-1 --stack-name clientidentity-service-prod --template-url ${templateUrl} \
  --capabilities CAPABILITY_IAM --capabilities CAPABILITY_NAMED_IAM --parameters \
  ParameterKey=environment,ParameterValue=${environment} \
  ParameterKey=vpcStackName,ParameterValue=${vpcStackName} \
  ParameterKey=albCommonStackName,ParameterValue=${albCommonStackName} \
